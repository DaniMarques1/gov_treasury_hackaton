...
for token in tokens:
    for fee_type in fee_types:
        ...
        for doc in result:
            ...
            if unix_timestamp not in final_results:
                final_results[unix_timestamp] = {
                    "timestamp": timestamp,
                    "date": unix_timestamp
                }
            final_results[unix_timestamp][f"{token.lower()}_{fee_type}"] = doc[f"{token.lower()}_{fee_type}"]
